...
func SetupDBTest() (DockerDBConn *gorm.DB, closeFunc func() error, err error) {
	...
	if err := pool.Retry(func() error {
		dataSource := fmt.Sprintf("postgres://%s:%s@%s/%s", cfg.Username, cfg.Password, cfg.Address, cfg.Database)
		DockerDBConn, err = gorm.Open(postgres.Open(dataSource))
		if err != nil {
			return err
		}

		tx := DockerDBConn.Exec("SELECT 1")
		return tx.Error
